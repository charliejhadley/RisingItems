...
# For getting many data files
library(furrr)


vec_country_codes <- codelist %>% 
  select(country.name.en, iso2c) %>% 
  drop_na() %>% 
  deframe()


get_irus_day_date <- function(selected_date, format){
...
download.file(get_irus_day_date("2024-01-01", "json"), "test-json.json")

data_json <- read_json("test-json.json")

names(data_json)

map_dfr(data_json$Statistics, ~as_tibble(.))

# Country

data_json$Statistics
